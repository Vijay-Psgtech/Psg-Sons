...
const Alumni2 = () => {
  ...
  useEffect(() => {
    lottie.loadAnimation({
      container: document.getElementById("swipeAnimation"),
      renderer: "svg",
      loop: true,
      autoplay: true,
      path: "/animations/Swipe Left.json", // file you downloaded
    });
  }, []);
  ...
  return (
    <div className="section min-h-screen w-full relative overflow-hidden">
      <img
        src="/images/bg2.jpg"
      ...
      <div
        className="relative z-10 flex flex-col items-center 
                                        px-4 sm:px-6 md:px-9 
                                            py-8 sm:py-16 md:py-24 lg:py-32"
                                                                                >
        {/* Dropdown */}
        <div className="relative mb-12 w-full max-w-md sm:max-w-md h-5 py-7">
          <button
            className="p-3 text-blue-800 text-sm border-b bg-white drop-shadow-lg rounded-lg flex items-center gap-2 w-full justify-between"
        ...
        <div className="relative w-full">
          {/* Navigation Buttons */}
          <div className="absolute right-4 sm:right-10 bottom-6 sm:bottom-12 flex gap-4 sm:gap-6 z-20">
            <button
              onClick={handleScrollLeft}
              className="w-10 h-10 sm:w-12 sm:h-12 flex justify-center items-center bg-white rounded-full drop-shadow-md"
            >
              <FaChevronLeft />
            </button>
            <button
          ...
          >
            {currentAlumni.map((alum, index) => (
              <div
                key={index}
                className="shrink-0 w-56 sm:w-60 md:w-72 rounded-lg p-5 sm:p-6 text-center bg-opacity-80 md"
              >
                <img
                  src={alum.image}
                  alt={alum.name}
        ...
        <div className="flex flex-col items-center mb-2 sm:hidden">
          <div id="swipeAnimation" className="w-28 h-28 "></div>
          <p className="text-sm text-gray-700 font-semibold text-center leading-tight">
            Swipe to <br /> Navigate
          </p>
        </div>
      </div>
    </div>
