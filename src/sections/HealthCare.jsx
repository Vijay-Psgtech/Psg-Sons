...
import { HealthCareData } from "../components/content/Content";
import {
  FaChevronRight,
  FaChevronLeft,
  FaHandPointRight,
...
const HealthCare = () => {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      if (window.location.hash === "#health-care" && scrollRef.current) {
          ...
          >
            <div className="ml-6 sm:ml-12 lg:ml-24 flex h-screen justify-center flex-col items-start shrink-0 w-[16rem] sm:w-[20rem]">
              ...
              <p className="mt-4 lg:mt-6 text-md sm:text-lg w-11/12 mb-36 whitespace-pre-wrap">
                PSG & Sons' Charities stands as a transformative force in
                Coimbatore's healthcare landscape. They have significantly
                elevated it through a multifaceted approach, encompassing
                hospital excellence, medical education, and paramedical
                training. The organisation's relentless commitment to progress,
                affordability, and accessibility has left an indelible mark and
                PSG & Sons' Charities stand as a cornerstone of positive change.
              </p>
            ...
            {HealthCareData.map((data, index) => (
              <div
                ...
                className={`h-[100dvh] drop-shadow-xl cursor-pointer group pointer-events-auto py-12 p-8 w-screen md:w-fit min-w-[15rem] max-w-[65rem] flex shrink-0 ${
                  index % 2 === 0
                    ? "items-start justify-start flex-col mt-24 ml-36 w-full false"
                    : "items-end -translate-y-24 justify-end  flex-col-reverse  md:-mx-24 false"
                }`}
              >
                <div
                  className={`rounded-xl flex gap-8 md:flex-row-reverse flex-col-reverse w-full ${
                    index % 2 === 0 ? "" : "mt-auto mb-0"
                  }`}
                >
